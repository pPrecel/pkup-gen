...
## Step 6: Done — generate report.txt and suggest next step
...
Where:
- `DD.MM.YYYY - DD.MM.YYYY` is the PKUP period established in Step 0
- `approvalDate` is the day after the period end date (e.g. period ends 18.01.2026 → approvalDate is 19.01.2026)
...
- The output `outputDir` contains `.diff` files and a `report.txt` with `period:` and `approvalDate:` pre-filled — ready for `/pkup-enchant` to write the `result:` section
